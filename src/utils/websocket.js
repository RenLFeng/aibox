...
import store from '@/store' // 与后端的协商，websocket请求需要带上token参数
...
export function websocketonmessage(e) {
  console.log('ws来消息了1111111111：', e.data)
...
function websocketclose(e) {
  ...
  // e.code !== 1000  表示非正常关闭。
  // console.log(e)
  console.log('全局alarmWebsocket连接关闭！')
  if (e && e.code !== 1000) {
    // Message.error('ws连接异常，请稍候重试')
    errorCallback()
  }
...
function websocketOpen(e) {
  console.log('全局alarmWebSocket连接成功')
}
...
function initWebSocket() {
  ...
  // ws请求完整地址
  const requstWsUrl = wsUrl + '/' + uuid.v4() + '/alarmDialog?token=' + token
