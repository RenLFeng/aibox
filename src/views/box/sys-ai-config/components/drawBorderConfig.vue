...
import { updateBorderLine } from '@/api/box/sys-ai-config'
  ...
  methods: {
    ...
    canvasUp(e) {
      ...
      if (this.lineType === 'rec') {
        ...
        //
        if (this.drawCount < 20) this.drawCount++
        else return
        const info = this.drawInfo[this.drawInfo.length - 1]
        // var x = info.x
        // var y = info.y
    ...
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          updateBorderLine(this.drawData).then(response => {
    ...
    updateData(resData, id) {
      this.$emit("updateItem", { id: id, borderLine: resData.configData });
    },
    // 关闭弹框的事件
    closeDialog() {
      this.reset()
