...
import { updateInterestedLine } from '@/api/box/sys-ai-config'
...
export default {
  ...
  methods: {
    ...
    canvasDown(e) {
      ...
      if (this.drawCount >= 1) {
        this.$message({
          message: '最多可设置1个感兴趣区，请删除后再绘制',
          type: 'error'
        })
        return
      }
      if (this.canDraw) {
        if (this.lineType === 'cer' && this.drawInfo.length > 51) {
          this.$message({
            message: '一个区域最多50个点',
            type: 'error'
          })
    ...
    submitForm: function() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          updateInterestedLine(this.drawData).then(response => {
    ...
    updateData(resData,id) {
      this.$emit('updateItem',{id:id, interestLine: resData.configData })
    },
    // 关闭弹框的事件
    closeDialog() {
      this.reset()
