...
export default {
  ...
  methods: {
    ...
    initDrawRec(data) {
      this.drawCount = 0
      if (data.configData.shieldLine) {
        const shieldArray = data.configData.shieldLine.split('|')
    ...
    canvasUp(e) {
      ...
      if (this.lineType === 'rec') {
        ...
        if (!this.formData.configData.shieldLine) {
          this.formData.configData.shieldLine = this.setRecData(info)
        } else {
          this.formData.configData.shieldLine += '|' + this.setRecData(info)
        }
      }
    ...
    clean() {
      ...
      this.drawInfo = []
      this.history = []
      this.formData.configData.shieldLine = ''
      this.drawCount = 0
    },
    // 多边形区域绘制完成事件
    setImgInfo() {
      ...
      if (this.lineType === 'cer') {
        ...
        this.drawInfo.forEach((item, index) => {
          this.formData.configData.shieldLine += item.x + ',' + item.y + ','
        })
        this.formData.configData.shieldLine = this.formData.configData.shieldLine.substring(
          0,
          this.formData.configData.shieldLine.length - 1
        )
        //
    ...
    revoke() {
      ...
      if (this.history.length > 0) {
        ...
        this.drawCount--
        const shieldArray = this.formData.configData.shieldLine.split('|')
        shieldArray.pop()
        this.formData.configData.shieldLine = shieldArray.join('|')
        if (this.lineType === 'cer') {
          this.drawInfo = []
        }
      }
