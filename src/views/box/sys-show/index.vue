    ...
    <div class="top-header">
      <div class="big-screen-title">
        ...
        <div class="top-header-tip">
          <div class="sub-title">此数据为实时真实数据</div>
                  ...
                  <div class="basic-header flex">
                    ...
                    <div class="top-basic-info">
                      <basic-data-item-label
                        ...
                        :in-cr-value="basicData.currentConfirmedIncr"
                      />
                      <basic-data-item-label
                        label="累计确诊"
                        :config="defaultDataConfig.confirmedCount"
                        :in-cr-value="basicData.confirmedIncr"
                      />
                      <!-- 境外输入 -->
                      <basic-data-item-label
                        label="境外输入"
                        :config="defaultDataConfig.importedCount"
                        :in-cr-value="basicData.importedIncr"
                      />
                      <!-- 无症状感染者 -->
                      <basic-data-item-label
                        label="无症状感染者"
                        :config="defaultDataConfig.noInFectCount"
                        :in-cr-value="basicData.noInFectIncr"
                      />
                      <!-- 累计治愈 -->
                      <basic-data-item-label
                        label="累计治愈"
                        :config="defaultDataConfig.curedCount"
                        :in-cr-value="basicData.curedIncr"
                      />
                      <!-- 死亡人数 -->
                      <basic-data-item-label
                        label="累计死亡"
                        :config="defaultDataConfig.deadCount"
                        :in-cr-value="basicData.deadIncr"
                      />
                    </div>
                    <!-- 顶部统计信息结束 -->
                  </div>
    ...
    <div class="province-data-table-wrapper">
      ...
      >
        <div slot="title" class="province-data-modal-title">
          <p>各省数据表</p>
          <span
            class="province-data-modal-update-time"
...
<script>
import ChartCard from '../../../components/echart/ChartCard'
import DataMap from '../../../components/echart/DataMap'
import CuredAndDeadRateChart from '../../../components/echart/CuredAndDeadRateChart'
import BasicDataItemLabel from '../../../components/echart/BasicDataItemLabel'
import BasicTrendChart from '../../../components/echart/BasicTrendChart'
import ProvinceRankingBarChart from '../../../components/echart/ProvinceRankingBarChart'
import CurrentConfirmedCompareBarChart from '../../../components/echart/CurrentConfirmedCompareBarChart'
// import About from '../../../components/echart/About'
import BasicProportionChart from '../../../components/echart/BasicProportionChart'

import covid19Service from '@/api/box/covid19'
...
const initBasicConfig = (data = null) => {
  const currentConfirmedCount = data ? [data.currentConfirmedCount] : 0
  const confirmedCount = data ? [data.confirmedCount] : 0
  const importedCount = data ? [data.importedCount] : 0
  const noInFectCount = data ? [data.noInFectCount] : 0
  const deadCount = data ? [data.deadCount] : 0
  const curedCount = data ? [data.curedCount] : 0
  return {
    confirmedCount: {
...
export default {
  ...
  methods: {
    ...
    setBasicIncrTrendData(data) {
      const dateList = []
      const currentConfirmedIncrList = []
      const importedIncrList = []

      const sevenDayDateList = []
      // 仅显示一周条数据
      const confirmedCountList = []
      const curedCountList = []
      // 仅获取7条数据
      const count = 7
      const noInFectDataList = []
    ...
    setProvinceComfirmedCountBoardData(areaList) {
      const resultList = areaList.map(item => {
        return [
          item.provinceLabel,
          item.confirmedCount,
          item.curedCount,
    ...
    setBasicData(data) {
      // 重新生成，否则视图不更新
      const config = initBasicConfig(data)
...
.chart-item-big-screen-container {
  margin: 0 10px;
}
.about-wraper {
  position: fixed;
